...
//			System.out.println("常熟: " + changshu.toString());
			String test3 = biaoshi.toString();
			if(test3.contains("void")){
				System.out.println("(关键字,void)");
				test3 = test3.replaceAll("void", "");
			}
			if(test3.contains("main")){
				System.out.println("(关键字,main)");
				test3 = test3.replaceAll(" main", "");
			}
			if(test3.contains("int")){
				System.out.println("(关键字,int)");
				test3 = test3.replaceAll(" int", "");
			}
			if(test3.contains("float")){
				System.out.println("(关键字,float)");
				test3 = test3.replaceAll(" float", "");
			}
			if(test3.contains("if")){
				System.out.println("(关键字,if)");
				test3 = test3.replaceAll(" if", "");
			}
			if(test3.contains("else")){
				System.out.println("(关键字,else)");
				test3 = test3.replaceAll(" else", "");
			}
			if(test3.contains("return")){
				System.out.println("(关键字,return)");
				test3 = test3.replaceAll(" return", "");
			}
//			System.out.println("标识符： " + test3);
			int e = 0;
			int xx = 0;
			for(int s = 0;s<test3.length();++s){
				if(test3.charAt(s) == ' '){
					xx = e;
					e = s;
					s = xx;
					String result = test3.substring(s, e);
					result.trim();
					if(!result.equals("")&&!result.equals(" ")){
						System.out.println("(标识符： " + result+")");	
					}
				}
			}
